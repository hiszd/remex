...
use remex_core::endpoint::Endpoint;
use thiserror::Error;
use tracing::error;
...
impl SessionMap {
  pub fn insert(&mut self, identity: Endpoint, addr: actix::Addr<session::RemexSession>) {
    let _ = self
      .sessions
  ...
  pub fn get_addr(&self, id: String) -> Result<actix::Addr<session::RemexSession>, Error> {
