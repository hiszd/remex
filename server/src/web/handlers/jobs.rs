...
use remex_core::db::model::server::jobs::{Job, NewJob, UpdateJob};
...
pub async fn get_jobs() -> impl Responder {
  ...
  let all_jobs = jobs::table.load::<Job>(&mut pool).unwrap();
...
pub async fn create_job(form: web::Json<CreateJobForm>) -> impl Responder {
  use remex_core::db::model::server::jobs_groups::NewJobGroup;
  ...
  let job = diesel::insert_into(jobs::table)
    .values(&new_job)
    .get_result::<Job>(&mut pool)
    .unwrap();

  if let Some(group_ids) = &form.group_ids {
    for gid in group_ids {
      let new_job_group = NewJobGroup {
...
#[post("/jobs/update")]
pub async fn update_job(job_update: web::Json<UpdateJob>) -> impl Responder {
...
pub async fn add_clients_to_jobs(actions: web::Json<Vec<JobClientAction>>) -> impl Responder {
  use remex_core::db::model::server::groups::NewGroup;
  use remex_core::db::model::server::groups_clients::NewGroupClients;
  use remex_core::db::model::server::jobs_groups::NewJobGroup;
  ...
  for action in actions.into_inner() {
    ...
    let group_id = if let Some(gid) = existing_group_id {
      ...
      let new_group_id = uuid::Uuid::now_v7().to_string();
