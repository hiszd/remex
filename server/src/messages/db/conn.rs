  ...
  fn handle(&mut self, msg: ClientAuth, ctx: &mut Context<Self>) -> Self::Result {
    ...
    match msg.authreq {
      AuthRequest::Secret(secret) => {
        let fut = async move {
          if secret == remex_core::SECRET {
            //see if there is a client with the machineid from msg.identity
            match db::query::client::get_client(&pool, m.identity.machineid.clone()).await {
              Ok(dbclient) => {
              ...
              Err(e) => {
                if let db::query::client::Error::Sqlx(sqlx::Error::RowNotFound) = e {
                  warn!("Valid secret, but client not found. Creating new one now");
                  match db::query::client::add_client(
                    &pool,
                    m.identity.machineid,
                    db::query::client::generate_id(pool.clone()).await.unwrap(),
                    m.identity.name.clone(),
                    db::query::client::generate_secret(),
                  )
                  .await
                  {
                    Ok(dbclient) => {
      ...
      AuthRequest::IdSecret(id, secret) => {
        let fut = async move {
          //see if there is a client with the machineid from msg.identity
          match db::query::client::get_client(&pool, id.clone()).await {
            Ok(dbclient) => {
