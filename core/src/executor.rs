...
use crate::db::{
  endpoint::model::executor::ExecutorModel,
  server::model::executor::ExecutorModel as ServExecutorModel,
};
...
impl From<ServExecutorModel> for Executor {
  fn from(e: ServExecutorModel) -> Self {
    Executor {
      id: e.id,
      name: e.name,
      command: e.command,
...
impl Into<ServExecutorModel> for Executor {
  fn into(self) -> ServExecutorModel {
    ServExecutorModel {
      id: self.id,
      name: self.name,
      command: self.command,
